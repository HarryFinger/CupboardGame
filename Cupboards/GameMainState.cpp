...
GameMainState::GameMainState(const std::string &path) : game_settings(path), cursor(PATH_TO_CURSOR_PIC)
{
    ...
    for (size_t i = 0; i < game_settings.GetChipsCount(); i++)
    {
        ...
        chips_container.push_back(std::make_unique<Chip>(
            sf::IntRect(MODULE * static_cast<uint32_t>(i), 0, MODULE, MODULE), sf::Vector2f(X, Y)));
    }
    ...
    for (size_t i = 0; i < game_settings.GetVerticalWiresVector().size(); i++)
    {
        ...
        vertical_wires_container.push_back(
            std::make_unique<Wire>(sf::IntRect(MODULE, 0, MODULE, MODULE), sf::Vector2f(X, Y)));
    }
    for (size_t i = 0; i < game_settings.GetHorisontalWiresVector().size(); i++)
    {
        ...
        horisontal_wires_container.push_back(
            std::make_unique<Wire>(sf::IntRect(0, 0, MODULE, MODULE), sf::Vector2f(X, Y)));
    }
...
void GameMainState::RenderWinPreviewMap(sf::RenderWindow &window)
{
    for (size_t i = 0; i < chips_container.size(); i++)
    {
        ...
        prev_sprite.setPosition(sf::Vector2f(win_condition_container[i].x * compression_ratio + offsetX,
                                             win_condition_container[i].y * compression_ratio + offsetY));
