...
void GameSettings::DivideWires()
{
    for (size_t i = 0; i < gamedata.number_of_wires; i++)
    {
        ...
        if (first_x == second_x)
        {
            gamedata.vertical_wires_coordinates.push_back(
                sf::Vector2f(static_cast<float>(first_x), static_cast<float>((first_y + second_y) / 2)));
        }
        if (first_y == second_y)
        {
            gamedata.horisontal_wires_coordinates.push_back(
                sf::Vector2f(static_cast<float>((first_x + second_x) / 2), static_cast<float>(first_y)));
        }
    }
}
