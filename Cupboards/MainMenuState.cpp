...
MainMenuState::MainMenuState() : cursor(PATH_TO_CURSOR_PIC)
{
    ...
    // 1st level
    button_container.emplace_back(
        std::make_unique<Button>(sf::IntRect(600, 0, 600, 210), sf::Vector2f(500.f, 130.f), PATH_TO_BUTTONS_PIC));
    button_container.emplace_back(
        std::make_unique<Button>(sf::IntRect(1200, 0, 600, 210), sf::Vector2f(500.f, 350.f), PATH_TO_BUTTONS_PIC));
    button_container.emplace_back(
        std::make_unique<Button>(sf::IntRect(1800, 0, 600, 210), sf::Vector2f(500.f, 570.f), PATH_TO_BUTTONS_PIC));
    // exit button
    button_container.emplace_back(
        std::make_unique<Button>(sf::IntRect(0, 0, 600, 210), sf::Vector2f(500.f, 850.f), PATH_TO_BUTTONS_PIC));
}
...
void MainMenuState::update(const float delta_time)
{
    ...
    if (button_container[2]->IsContains(cursor.getPosition()))
    {
        ...
        button_container[2]->StopFlicker();
    ...
    if (button_container[3]->IsContains(cursor.getPosition()))
    {
        ...
        button_container[3]->StopFlicker();
